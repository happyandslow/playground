...
1.  Input/output tensor names
1.  Output shape inference logic (from inputs and attributes)
1.  Operator attributes (if any)

### Recommended for optimization
1.  FLOP count calculation (for cost estimation)
1.  Memory requirements (output size + temporary buffers)
...
1.  Hardware-specific predictors (ML models)
1.  Special handling in mapping algorithms
1.  Execution semantics (for graph execution)
...
1.  The executor collects all tensors with is_output=True

1.  Multiple outputs are natively supported

1.  A sink operator adds overhead without benefit:
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwMTQyMjgyNDUsODE4NjM2MTI5LC0yMD
E1NTY2ODYyLDE0ODgwNjQxODQsLTIwMDcxNDU3MTMsLTc5NTQz
NzI4M119
-->