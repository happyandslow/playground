...
def main():
    ...
    # ---------------------------------------------------------------------- #
    tenants = getattr(config, "tenants", None)
    if tenants is None:
        tenants = [{"name": "t0", "x": 0, "y": 0}]

    # Basic overlap check and layout extents (in PE coordinates)
    ...
    for i in range(len(tenants)):
        for j in range(i + 1, len(tenants)):
            if blocks_overlap(tenants[i], tenants[j]):
                raise ValueError(f"Tenants {tenants[i].get('name', i)} and {tenants[j].get('name', j)} overlap")

    layout_width = max(t["x"] + P for t in tenants)
    layout_height = max(t["y"] + P for t in tenants)
    ...
    print(
        ...
        f"ffn_dim_p_pe: {ffn_dim_p_pe}, num_tenants: {len(tenants)}, "
        f"layout_width: {layout_width}, layout_height: {layout_height}"
    )
    ...
    for t in tenants:
        runner.memcpy_h2d(
            sym_freqs_sin, freqs_sin_u32,
            t["x"], t["y"], P, P, _dim_p_pe//2,
            streaming=False, data_type=io_dtype, order=memcpy_order, nonblock=False
        )
    # Copy freqs_cos
    ...
    for t in tenants:
        runner.memcpy_h2d(
            sym_XKCache, XKCache_u32,
            t["x"], t["y"], P, P, dim_p_pe * seq_len_p_pe,
            streaming=False, data_type=io_dtype, order=memcpy_order, nonblock=False
        )
    # Copy XVCache
    ...
    for t in tenants:
        runner.memcpy_h2d(
            sym_XVCache, XVCache_u32,
            t["x"], t["y"], P, P, seq_len_p_pe * dim_p_pe,
            streaming=False, data_type=io_dtype, order=memcpy_order, nonblock=False
        )
    # Copy O_weight
    ...
    for t in tenants:
        runner.memcpy_h2d(
            sym_UP_weight, UP_u32,
            t["x"], t["y"], P, P, dim_p_pe * ffn_dim_p_pe,
            streaming=False, data_type=io_dtype, order=memcpy_order, nonblock=False
        )
    # Copy GATE_weight
    ...
    for t in tenants:
        runner.memcpy_h2d(
            sym_DOWN_weight, DOWN_u32,
            t["x"], t["y"], P, P, ffn_dim_p_pe * dim_p_pe,
            streaming=False, data_type=io_dtype, order=memcpy_order, nonblock=False
        )
    ...
    # Read back the full logical program rectangle and then print out a
    ...
    # Shape: [layout_height, layout_width, bsz * dim_p_pe]
    debug = debug.reshape(layout_height, layout_width, bsz * dim_p_pe)
    
    # -------------------------------------------------------------------------- #
    # ------------------------------ Timer Check ------------------------------ #
    # -------------------------------------------------------------------------- #
    # Copy back timer_buf from all width x height PEs
    timer_buf_1d_u32 = np.zeros((layout_width*layout_height*3), dtype=np.uint32)
    runner.memcpy_d2h(
        timer_buf_1d_u32, symbol_timer_buf, 0, 0, layout_width, layout_height, 3, streaming=False,
        data_type=MemcpyDataType.MEMCPY_32BIT, order=MemcpyOrder.ROW_MAJOR, nonblock=False
    )
    timer_buf_time_hwl = timer_buf_1d_u32.view(np.float32).reshape((layout_height, layout_width, 3))
    ...
    # ------------------------------ Compute time ------------------------------ #
    # -------------------------------------------------------------------------- #
    cycles_count = np.zeros((layout_height, layout_width))
    for pe_x in range(layout_width):
        for pe_y in range(layout_height):
            cycles_count[pe_y, pe_x] = calculate_cycles(timer_buf_time_hwl[pe_y, pe_x, :])
    
    # Report per-tenant mean cycles over each tenant's P x P block.
    for idx, t in enumerate(tenants):
        name = t.get("name", f"t{idx}")
        y0 = t["y"]
        x0 = t["x"]
        block = cycles_count[y0:y0 + P, x0:x0 + P]
        mean_cycles = block.mean()
        print(f"Host: mean cycles count {name}: {mean_cycles / repeat_steps}")
