...
def main():
    ...
    with SdkRuntime(artifact_path, simulator=args.simulator) as runner:
        ...
        # Tenant 1
        runner.memcpy_h2d(
            sym_X, X_u32, P, 0, P, P, bsz*dim_p_pe, streaming=False, data_type=io_dtype, order=memcpy_order, nonblock=False
        )
        ...
        runner.memcpy_h2d(
            sym_W, W_u32, P, 0, P, P, dim_p_pe, streaming=False, data_type=io_dtype, order=memcpy_order, nonblock=False
        )
        ...
        runner.memcpy_h2d(
            sym_Q_weight, Q_u32, P, 0, P, P, dim_p_pe * dim_p_pe, streaming=False, data_type=io_dtype, order=memcpy_order, nonblock=False
        )
        ...
        runner.memcpy_h2d(
            sym_K_weight, K_u32, P, 0, P, P, dim_p_pe * dim_p_pe, streaming=False, data_type=io_dtype, order=memcpy_order, nonblock=False
        )
        ...
        runner.memcpy_h2d(
            sym_V_weight, V_u32, P, 0, P, P, dim_p_pe * dim_p_pe, streaming=False, data_type=io_dtype, order=memcpy_order, nonblock=False
        )
        ...
        runner.memcpy_h2d(
            sym_freqs_sin, freqs_sin_u32, P, 0, P, P, _dim_p_pe//2, streaming=False, data_type=io_dtype, order=memcpy_order, nonblock=False
        )
        # Copy freqs_cos
        ...
        runner.memcpy_h2d(
            sym_freqs_cos, freqs_cos_u32, P, 0, P, P, _dim_p_pe//2, streaming=False, data_type=io_dtype, order=memcpy_order, nonblock=False
        )
        # Copy XKCache
        ...
        runner.memcpy_h2d(
            sym_XKCache, XKCache_u32, P, 0, P, P, dim_p_pe * seq_len_p_pe, streaming=False, data_type=io_dtype, order=memcpy_order, nonblock=False
        )
        # Copy XVCache
        ...
        runner.memcpy_h2d(
            sym_XVCache, XVCache_u32, P, 0, P, P, seq_len_p_pe * dim_p_pe, streaming=False, data_type=io_dtype, order=memcpy_order, nonblock=False
        )
        # Copy O_weight
        ...
        runner.memcpy_h2d(
            sym_O_weight, O_u32, P, 0, P, P, dim_p_pe * dim_p_pe, streaming=False, data_type=io_dtype, order=memcpy_order, nonblock=False
        )
        # Copy UP_weight
        ...
        runner.memcpy_h2d(
            sym_UP_weight, UP_u32, P, 0, P, P, dim_p_pe * ffn_dim_p_pe, streaming=False, data_type=io_dtype, order=memcpy_order, nonblock=False
        )
        # Copy GATE_weight
        ...
        runner.memcpy_h2d(
            sym_GATE_weight, GATE_u32, P, 0, P, P, dim_p_pe * ffn_dim_p_pe, streaming=False, data_type=io_dtype, order=memcpy_order, nonblock=False
        )
        # Copy DOWN_weight
        ...
        runner.memcpy_h2d(
            sym_DOWN_weight, DOWN_u32, P, 0, P, P, ffn_dim_p_pe * dim_p_pe, streaming=False, data_type=io_dtype, order=memcpy_order, nonblock=False
        )
        ...
        runner.memcpy_d2h(
            debug_1d_u32, sym_debug, 0, 0, 2 * P, P, bsz * dim_p_pe, streaming=False, data_type=io_dtype, order=memcpy_order, nonblock=False
        )
        ...
        runner.memcpy_d2h(
            timer_buf_1d_u32, symbol_timer_buf, 0, 0, 2 * P, P, 3, streaming=False,
            data_type=MemcpyDataType.MEMCPY_32BIT, order=MemcpyOrder.ROW_MAJOR, nonblock=False
        )
        timer_buf_time_hwl = timer_buf_1d_u32.view(np.float32).reshape((P, 2 * P, 3))
        
    # -------------------------------------------------------------------------- #
    # ------------------------------ Compute time ------------------------------ #
    # -------------------------------------------------------------------------- #
    cycles_count = np.zeros((P, 2 * P))
    for pe_x in range(2 * P):
            for pe_y in range(P):
