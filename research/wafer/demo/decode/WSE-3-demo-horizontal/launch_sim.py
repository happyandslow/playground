...
def main():
    ...
    # Read back debug from the full 2P x P region (two tenants).
    ...
    # Copy back timer_buf from all width x height PEs (2P x P)
    ...
    cycles_t0 = cycles_count[:, :P].mean()
    cycles_t1 = cycles_count[:, P:].mean()
    print(f"Host: mean cycles count tenant 0: {cycles_t0/repeat_steps}")
    print(f"Host: mean cycles count tenant 1: {cycles_t1/repeat_steps}")
