...
y3_symbol = runner.get_id('y3')
y4_symbol = runner.get_id('y4')
...
for step in range(2):
  ...
  runner.memcpy_h2d(A_symbol, np.tile(A, width), width, 1, width, 1, M*N, streaming=False,
    order=MemcpyOrder.ROW_MAJOR, data_type=MemcpyDataType.MEMCPY_32BIT, nonblock=False)
  runner.memcpy_h2d(x_symbol, np.tile(x, width), width, 1, width, 1, N, streaming=False,
    order=MemcpyOrder.ROW_MAJOR, data_type=MemcpyDataType.MEMCPY_32BIT, nonblock=False)
  runner.memcpy_h2d(b_symbol, np.tile(b, width), width, 1, width, 1, M, streaming=False,
    order=MemcpyOrder.ROW_MAJOR, data_type=MemcpyDataType.MEMCPY_32BIT, nonblock=False)
  ...
  np.testing.assert_allclose(y4_result, np.tile(y_expected, width), atol=0.01, rtol=0)
  print("SUCCESS!")
